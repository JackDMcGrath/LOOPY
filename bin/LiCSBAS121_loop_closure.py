...
def main(argv=None):
    ...
    global Aloop, ifgdates, ifgdir, length, width, loop_pngdir, cycle, \
        multi_prime, bad_ifg, noref_ifg, bad_ifg_all, refy1, refy2, refx1, refx2, \
        corrdir, la  ## for parallel processing
    ...
    corrdir = ifgdir + 'MedCorr'
    if not os.path.exists(os.path.join(corrdir)): os.mkdir(os.path.join(corrdir))
    ...
    # create 3D cube, containing modulo nPi phase error
    a = np.full((length,width,len(Aloop)), np.nan) #, dtype=bool)
    la = xr.DataArray(
        data=a,
        dims=[ "y", "x", "nLoop"],
        coords=dict(y=np.arange(length),x=np.arange(width),nLoop=np.arange(len(Aloop))))
    ...
    ns_loop_err, da, la = loop_closure_4th([0, len(Aloop)], da, la)
    #ns_loop_err = np.sum(res[:, :, :,], axis=0)


    med_correct = True
    para_correct = True
    if med_correct and para_correct:
        print('Trying to correct based of median error')

        p = q.Pool(_n_para)
        p.map(median_correct_loops,range(Aloop.shape[1]))
        p.close()

    elif med_correct and not para_correct:
        print('Trying to correct based of median error')
        for ix, ifg in enumerate(ifgdates):
            if not os.path.exists(os.path.join(corrdir, ifg)): os.mkdir(os.path.join(corrdir, ifg))
            if np.mod(ix, 10) == 0:
                print("  {0:3}/{1:3}th loop...".format(ix, n_loop), flush=True)
                go = time.time()
                prev = go
                stagen = 0
            unwloops = np.where(Aloop[:, ix] != 0)[0]
            if np.mod(ix, 10) == 0:
                stage = time.time()
                stagen += 1
                print('Step {} Time: {:.5f}s\t Total: {:.5f}s'.format(stagen, stage-prev, stage-go))
                prev = stage
            unwfile = os.path.join(ifgdir, ifg, ifg+'.unw')
            if np.mod(ix, 10) == 0:
                stage = time.time()
                stagen += 1
                print('Step {} Time: {:.5f}s\t Total: {:.5f}s'.format(stagen, stage-prev, stage-go))
                prev = stage
            data = io_lib.read_img(unwfile, length, width)
            if np.mod(ix, 10) == 0:
                stage = time.time()
                stagen += 1
                print('Step {} Time: {:.5f}s\t Total: {:.5f}s'.format(stagen, stage-prev, stage-go))
                prev = stage
            corr = np.nanmedian(la[:, :, unwloops] * Aloop[unwloops, ix], axis=2).round()
            if np.mod(ix, 10) == 0:
                stage = time.time()
                stagen += 1
                print('Step {} Time: {:.5f}s\t Total: {:.5f}s'.format(stagen, stage-prev, stage-go))
            ...
            if np.mod(ix, 10) == 0:
                ...
                print('Step {} Time: {:.5f}s\t Total: {:.5f}s'.format(stagen, stage-prev, stage-go))
                prev = stage
            data3 = [np.angle(np.exp(1j*(data/3))*3), corr, np.angle(np.exp(1j*((data-(corr*np.pi))/3))*3)]
            if np.mod(ix, 10) == 0:
                stage = time.time()
                stagen += 1
                print('Step {} Time: {:.5f}s\t Total: {:.5f}s'.format(stagen, stage-prev, stage-go))
                prev = stage
            pngfile = os.path.join(corrdir, ifg, ifg+'_corr.png')
            if np.mod(ix, 10) == 0:
                stage = time.time()
                stagen += 1
                print('Step {} Time: {:.5f}s\t Total: {:.5f}s'.format(stagen, stage-prev, stage-go))
                prev = stage
            title3 = ['unw_org', 'corr', 'unw_corr']
            if np.mod(ix, 10) == 0:
                stage = time.time()
                stagen += 1
                print('Step {} Time: {:.5f}s\t Total: {:.5f}s'.format(stagen, stage-prev, stage-go))
                prev = stage
            plot_lib.make_3im_png(data3, pngfile, SCM.romaO, title3, vmin=-np.pi, vmax=np.pi, cbar=True)
            if np.mod(ix, 10) == 0:
                stage = time.time()
                stagen += 1
                print('Step {} Time: {:.5f}s\t Total: {:.5f}s'.format(stagen, stage-prev, stage-go))
                prev = stage
            (data - (corr * np.pi)).astype(np.float32).tofile(os.path.join(corrdir, ifg, ifg + '.unw'))
            if np.mod(ix, 10) == 0:
                stage = time.time()
                stagen += 1
                print('Step {} Time: {:.5f}s\t Total: {:.5f}s'.format(stagen, stage-prev, stage-go))
...
def loop_closure_4th(args, da, la):
    ...
    for i in range(i0, i1):
        ...
        ## Calculate loop phase taking into account ref phase
        loop_ph = unw12+unw23-unw13-(ref_unw12+ref_unw23-ref_unw13)
        # Add modulo npi to array
        la[:, :, i] = (loop_ph / np.pi).round()
        ## Count number of loops with suspected unwrap error (>pi)
        loop_ph[np.isnan(loop_ph)] = 0 #to avoid warning
...
def median_correct_loops(i):
    if np.mod(i, 25) == 0:
        print("  {0:3}/{1:3}th ifg...".format(i, Aloop.shape[1]), flush=True)
    ifg = ifgdates[i]
    if not os.path.exists(os.path.join(corrdir, ifg)):
        os.mkdir(os.path.join(corrdir, ifg))
    if not os.path.exists(os.path.join(corrdir, ifg, ifg + '.cc')):
        os.symlink(os.path.join(ifgdir, ifg, ifg + '.cc'), os.path.join(corrdir, ifg, ifg + '.cc'))
    unwloops = np.where(Aloop[:, i] != 0)[0]
    unwfile = os.path.join(ifgdir, ifg, ifg+'.unw')
    data = io_lib.read_img(unwfile, length, width)
    corr = np.nanmedian(la[:, :, unwloops] * Aloop[unwloops, i], axis=2).round()
    corr[np.where(np.isnan(corr))] = 0
#    if ifg == '20200229_20200405':
#        print(unwloops)
#        for i in unwloops:
#            print(np.unique(la[:,:,i], return_counts=True))
#        print(np.unique(corr, return_counts=True))
    ...
    unwpngfile = os.path.join(corrdir, ifg, ifg +'.unw.png')
    plot_lib.make_im_png(np.angle(np.exp(1j*(data - (corr * np.pi))/cycle)*cycle), unwpngfile, SCM.romaO, ifg +'.unw', vmin=-np.pi, vmax=np.pi, cbar=False)
