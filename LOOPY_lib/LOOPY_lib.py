...
def make_compare_png(uncorr, corrunw, npi, corr, png, titles4, cycle):
    ...
    corr[np.where(np.isnan(npi))] = np.nan
...
# %%
def make_filt_png(uncorr, corrunw, npi, corr, png, titles4, cycle):
    """
    Make 4 panel png to compare uncorrected and corrected unw, also showing
    original modulo npi and correction
    """
    ...
    # npi
    ax = fig.add_subplot(2, 2, 3)  # index start from 1
    im = ax.imshow(npi, vmin=np.nanmin(npi), vmax=np.nanmax(npi), cmap='tab20c', interpolation='nearest')
    ax.set_title('{}'.format(titles4[2]))
    ax.set_xticklabels([])
    ax.set_yticklabels([])
    cax = plt.colorbar(im)
    ...
    # Correction
    ax = fig.add_subplot(2, 2, 4)  # index start from 1
    im = ax.imshow(corr, vmin=np.nanmin(npi), vmax=np.nanmax(npi), cmap='tab20c', interpolation='nearest')
    ax.set_title('{}'.format(titles4[3]))
    ax.set_xticklabels([])
    ax.set_yticklabels([])
    cax = plt.colorbar(im)
